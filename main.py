...
app = FastAPI(
    ...
    description="A simple API to manage addresses and retrieve them based on proximity using geodesic distance.",
    version="1.0.0"
)
...
def create_address(address: schemas.AddressCreate, db: Session = Depends(get_db)):
    """Add a new address with coordinates to the address book."""
...
def read_addresses(skip: int = Query(0), limit: int = Query(100), db: Session = Depends(get_db)):
    """Retrieve a list of all stored addresses with pagination support."""
...
def read_address(address_id: int, db: Session = Depends(get_db)):
    """Get details of a specific address by its unique ID."""
...
def update_address(address_id: int, address: schemas.AddressUpdate, db: Session = Depends(get_db)):
    """Update coordinates or details for an existing address entry."""
...
def delete_address(address_id: int, db: Session = Depends(get_db)):
    """Remove an address entry from the database."""
...
def search_addresses(
    ...
    dist: float = Query(..., gt=0, description="Search radius in kilometers"),
    db: Session = Depends(get_db)
):
    """
    Find addresses within a certain distance from a given point.
    Uses the geodesic distance (great-circle) for accurate proximity results.
    """
...
if __name__ == "__main__":
    import uvicorn
    # Hosting on 127.0.0.1 for local browser access
